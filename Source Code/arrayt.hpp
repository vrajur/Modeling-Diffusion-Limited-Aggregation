...
public:
	...
	arrayt( const int n1 );					// for 1D vector style
	arrayt( const int n1, const int n2 );	// for 2D matrix style
	arrayt( const arrayt<T> &a );

	//  destructor function
	inline ~arrayt() { delete [] p; nn=nndim=nn1=nn2=0; }
...
{
	if( n1 <= 0  ) {
		printf("arrayt initialized with size = %d, NOT ALLOWED\n", n1 );
		exit( EXIT_FAILURE );
	}
	p = new T [ n1 ];
	if( NULL == p ) {
		printf( "Cannot allocate memory for arrayt size = %d\n", n1 );
		exit( EXIT_FAILURE );
	}
	nn = nn1 = n1;
	nn2 = 0;
...
{
	if( ( n1 <= 0 ) || ( n2 <= 0 ) ) {
		printf("arrayt initialized with size = %d x %d, NOT ALLOWED\n",
			n1, n2 );
		exit( EXIT_FAILURE );
	}
	p = new T [ n1*n2 ];
	if( NULL == p ) {
		printf( "Cannot allocate memory for arrayt size = %d x %d\n",
			n1, n2 );
		exit( EXIT_FAILURE );
	}
	nn1 = n1;
...
{
	if( n <= 0  ) {
		printf("arrayt resize() with size = %d, NOT ALLOWED\n",
			n );
		exit( EXIT_FAILURE );
	}
	delete [] p;
	p = new T [ n ];
	if( NULL == p ) {
		printf( "Cannot allocate memory for arrayt resize = %d\n",
			n );
		exit( EXIT_FAILURE );
	}
	nn1 = n;
...
{
	if( ( n1 <= 0 ) || ( n2 <= 0 ) ) {
		printf("arrayt resize() with size = %d x %d, NOT ALLOWED\n",
			n1, n2 );
		exit( EXIT_FAILURE );
	}
	delete [] p;
	p = new T [ n1 * n2 ];
	if( NULL == p ) {
		printf( "Cannot allocate memory for arrayt resize = %d x %d\n",
			n1, n2 );
		exit( EXIT_FAILURE );
	}
	nn1 = n1;
...
arrayt<T>& arrayt<T>::operator=( const arrayt<T> &m )
{
	if( (nn != m.nn) || (nndim != m.nndim)  ){
		printf( "arrayt = operator invoked with unequal sizes\n"
			"  m1 size = %d, dim= %d\n"
...
inline T& arrayt<T>::operator()( const int i1, const int i2 )
{
	...
	if( (i1<0) || (i1>=nn1) ||
		(i2<0) || (i2>=nn2) || (nndim != 2 ) ){
		printf( "out of bounds index in arrayt\n"
			"  size = %d x %d, ndim= %d\n  access = (%d , %d)\n",
			nn1, nn2, nndim, i1, i2 );
		exit( EXIT_FAILURE );
	}
